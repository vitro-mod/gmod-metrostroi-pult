...
function ENT:CreateModels()
    ...
    if not self.ARSOnly then
        ...
        if self.Left or self.Double then self:SpawnMainModels(self.BasePos[self.LightType] * vector_mirror,
                Angle(0, 180, 0), LenseNum, self.Double and "d" or nil) end
        ...
        if self.RouteNumbers.sep and self.RouteNumbers[self.RouteNumbers.sep][1] ~= "X" then
            ...
            if self.Left then self.RouteNumbers[id].pos = self.RouteNumbers[id].pos * vector_mirror +
                TLM.LampIndicator[4] end
        ...
        for _, v in ipairs(self.LensesTBL) do
            ...
            if not self.Left or self.Double then self:SpawnHead(ID .. (#v + ID2), head,
                    self.BasePos[self.LightType] + offsetAndLongOffset, angle_zero, false, #v == 1) end
            if self.Left or self.Double then self:SpawnHead((self.Double and ID .. (#v + ID2) .. "d" or ID), head,
                    (self.BasePos[self.LightType] + offsetAndLongOffset) * vector_mirror, angle_zero, true, #v == 1) end
            ...
            if v ~= "M" and v ~= "X" then
                for i = 1, #v do
                    ...
                    if assembled and i < #v then
                        if not self.Left or self.Double then self:SpawnHead(ID .. ID2, head,
                                self.BasePos[self.LightType] + offsetAndLongOffset + TLM['step'] * (#v - i), angle_zero,
                                false, i == #v - 1) end
                        if self.Left or self.Double then self:SpawnHead((self.Double and ID .. ID2 .. "d" or ID .. ID2),
                                head,
                                (self.BasePos[self.LightType] + offsetAndLongOffset) * vector_mirror + TLM['step'] *
                                (#v - i), angle_zero, true, i == #v - 1) end
                    end
...
function ENT:UpdateModels(CurrentTime)
    ...
    for k, v in pairs(self.RouteNumbers) do
        ...
        if not IsValid(self.Models[3][rou1k]) and State1 > 0 then
            self.Models[3][rou1k] = ClientsideModel(
            v[3] and LampIndicatorModels_numb_mdl or LampIndicatorModels_lamp_mdl, RENDERGROUP_OPAQUE)
    ...
    if self.Arrow then
        ...
        if not IsValid(self.Models[3]["roua"]) and State > 0 then
            ...
            self.SpecRouteNumbers.pos = (self.BasePos[self.LightType] + offset - TLM.SpecRouteNumberOffset) -
            (self.RouteNumberOffset or vector_origin) + TLM.RouteNumberOffset3
