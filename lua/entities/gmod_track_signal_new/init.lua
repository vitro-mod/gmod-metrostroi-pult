...
function ENT:OpenRoute(route)
	...
	for i1 = 1, #Switches do
		...
		if not Metrostroi.GetSwitchByName(SwitchName) then
			print(self.Name, "switch not found")
			continue
		end
...
function ENT:PostInitalize()
	...
	for k, v in ipairs(self.Routes) do
		if v.NextSignal == "*" and self.TrackPosition then
			...
			local cursig = self
			while true do
				cursig = Metrostroi.GetARSJoint(cursig.TrackPosition.node1, cursig.TrackPosition.x, cursig.TrackDir,
					false)
...
function ENT:GetARS(ARSID, Force1_5, Force2_6)
	...
	return self.ARSSpeedLimit == ARSID or
	((self.TwoToSix and not Force1_5 or Force2_6) and nxt and nxt == ARSID and self.ARSSpeedLimit > nxt)
end
...
function ENT:GetRS()
	...
	return (self.ARSSpeedLimit > 4 or self.ARSSpeedLimit == 4 and self.Approve0) and
	(not self.ARSNextSpeedLimit or self.ARSNextSpeedLimit >= self.ARSSpeedLimit)
end
...
function ENT:CheckOccupation()
	...
	if not self.Close and not self.KGU then --not self.OverrideTrackOccupied and
		if self.Node and self.TrackPosition then
			self.Occupied, self.OccupiedBy, self.OccupiedByNow = Metrostroi.IsTrackOccupied(self.Node,
				self.TrackPosition.x, self.TrackPosition.forward, self.ARSOnly and "ars" or "light", self)
		end
...
function ENT:ARSLogic(tim)
	...
	if self.Routes[self.Route] then
		...
		elseif self.Routes[self.Route].ARSCodes then
			...
			self.ARSNextSpeedLimit = IsValid(self.NextSignalLink) and self.NextSignalLink.ARSSpeedLimit or
			tonumber(ARSCodes[1])
	...
	if not self.ControllerLogic then
		...
		if self.MU or self.ARSOnly or self.RouteNumberSetup and self.RouteNumberSetup ~= "" or self.RouteNumber and self.RouteNumber ~= "" then
			if self.NextSignalLink then
				if not self.NextSignalLink.Red and not self.Red then
					self.RouteNumberOverrite = self.NextSignalLink.RouteNumberOverrite ~= "" and
					self.NextSignalLink.RouteNumberOverrite or self.NextSignalLink.RouteNumber
				else
		...
		for k, v in ipairs(self.Lenses) do
			...
			if v ~= "M" then
				...
				local data = #v ~= 0 and self.TrafficLightModels[self.SignalType][#v - 1] or
				self.TrafficLightModels[self.SignalType][self.Signal_IS]
				-- if not data then continue end
				for i = 1, #v do
					--Get the LightID and check, is this light must light up
					local LightID = IsValid(self.NextSignalLink) and math.min(#Route.LightsExploded, self.FreeBS + 1) or
					1
					local AverageState = Route.LightsExploded[LightID]:find(tostring(index)) or
					((v[i] == "W" and self.InvationSignal and self.GoodInvationSignal == index) and 1 or 0)
					local MustBlink = (v[i] == "W" and self.InvationSignal and self.GoodInvationSignal == index) or
					...
					if AverageState > 0 then
						if self.GoodInvationSignal ~= index then self.Colors = self.Colors ..
							(MustBlink and v[i]:lower() or v[i]:upper()) end
						if v[i] == "R" then
		...
		for k, v in ipairs(self.Lenses) do
			if v ~= "M" then
				--get the some models data
				local data = #v ~= 1 and self.TrafficLightModels[self.SignalType][#v - 1] or
				self.TrafficLightModels[self.SignalType][self.Signal_IS]
				if not data then continue end
				for i = 1, #v do
