...
VitroMod.Rays.atLook = function(trace, ply, name, userOffset, userAngle)
    ...
    local pos = trace.HitPos + offset + userOffset
    local ang = angles + userAngle
    local tr = Metrostroi.RerailGetTrackData(trace.HitPos,ply:GetAimVector())
    if tr then pos = tr.centerpos + offset + userOffset end
...
hook.Add("PlayerSpawn", 'VitroMod.Rays', function(ply)
