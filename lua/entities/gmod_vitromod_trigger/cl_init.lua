...
local freeColor = Color(255, 200, 0)
local occColor = Color(255, 0, 0)
local dopColor = Color(255, 255, 255)
local dop_length = 100
    ...
    for k,v in pairs(VitroMod.Triggers) do
		for k2,v2 in pairs(v) do
			if k2 ~= "occ" then
				...
				local col = v.occ and occColor or freeColor
				if v2.pos and v2.angle and v2.min and v2.max then
					render.DrawWireframeBox(v2.pos,v2.angle,v2.min,v2.max, col)
					render.DrawWireframeBox(v2.pos, v2.angle, v2.min + Vector(20, -dop_length, 0), v2.max + Vector(-20, dop_length, 0), dopColor)
				end
